...
def get_data_loaders(train_batch_size, test_batch_size):
    ...
    train_loader = DataLoader(CIFAR10(path, download=False, transform=transform_train, train=True),
                              batch_size=train_batch_size, shuffle=True, num_workers=1)

    test_loader = DataLoader(CIFAR10(path, download=False, transform=transform_test, train=False),
...
def trainer(network, train_loader, device, epochs):
    ...
    lr = 0.1 for epoch <83
    lr = 0.01 for epoch 83-123
    lr = 0.001 for epoch >=124
    lr = 0.01 for first epoch to warm up training if network is deep
    """
    scheduler = MultiStepLR(optimizer, milestones=[83, 124], gamma=0.1)

    print('start training of ', epochs, " epochs")
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        if epoch % 10 == 9:# or epoch == 4:
            validate(network, train_loader, device, epoch+1, t0=t0)

    error = validate(network, train_loader, device, epochs)
    print('Train acc ', 100-error, '. Training time ', (time.perf_counter() - t0) / 60, ' min')
...
def ResNet_function(X):
    ...
    error = trainer(my_net, train_loader, device, epochs=epochs)

    validate(my_net, test_loader, device, -1)
...
def main():
    X = dict()
    X["block"] = 1          # if best_arch.block == "basic" else 1
    X["group_size"] = 2    # best_arch.group_size
    X["n_filters"] = 1     # best_arch.n_filters
    X["filter_upd"] = 1     # best_arch.filter_upd
    X["n_blocks1"] = 0      # best_arch.n_blocks1
    X["n_blocks2"] = 0    # best_arch.n_blocks2
    X["n_blocks3"] = 0    # best_arch.n_blocks3
    X["n_blocks4"] = 0    # best_arch.n_blocks4
    X["conv0"] = 0
    X["pool"] = 0
    X["reduce"] = 0
    X["epochs"] = 164
